	...
	<div id="wrapper" class="toggled">
		...
		<table class="table table-bordered">
			...
			<c:forEach var="table" items="${list}">
				<tr>
					...
					<td>${table.acceptableNum }</td>
					<td>${table.status == 1 ?'使用中':'未使用' }</td>
				
					<td>
					<a href="#" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#staffEditDialog" onclick= "editStaff(${staff.staffId})">查看订单</a>
					<a class="btn btn-danger btn-xs" href="food/foodList.do?tableNum=${table.tableNum }">使用</a>
					</td>
				</tr>
		...
		<div class="pager">
			<font size="2">共 ${page.totalPageCount} 页</font> <font size="2">第
				${page.pageNow} 页</font> <a href="table/tableList.do?pageNow=1">首页</a>
			<c:choose>
				<c:when test="${page.pageNow - 1 > 0}">
					<a href="table/tableList.do?pageNow=${page.pageNow - 1}">上一页</a>
				</c:when>
				<c:when test="${page.pageNow - 1 <= 0}">
					<a href="table/tableList.do?pageNow=1">上一页</a>
				</c:when>
			</c:choose>
			<c:choose>
				<c:when test="${page.totalPageCount==0}">
					<a href="table/tableList.do?pageNow=${page.pageNow}">下一页</a>
				</c:when>
				<c:when test="${page.pageNow + 1 < page.totalPageCount}">
					<a href="table/tableList.do?pageNow=${page.pageNow + 1}">下一页</a>
				</c:when>
				<c:when test="${page.pageNow + 1 >= page.totalPageCount}">
					<a href="table/tableList.do?pageNow=${page.totalPageCount}">下一页</a>
				</c:when>
			</c:choose>
			<c:choose>
				<c:when test="${page.totalPageCount==0}">
					<a href="table/tableList.do?pageNow=${page.pageNow}">尾页</a>
				</c:when>
				<c:otherwise>
					<a href="table/tableList.do?pageNow=${page.totalPageCount}">尾页</a>
				</c:otherwise>
			</c:choose>
		</div>
